...
class Controller extends \yii\web\Controller {
    ...
    public $whiteFooter = false;
    public $blackHeader = false;
    ...
    public function render($view, $params = [], $pjaxForce = true) {
        ...
        if ($pjaxForce && $this->hasMethod("action".ucfirst($view))) {
            $method = new \ReflectionMethod($this, "action".ucfirst($view));
            $args = [];

            foreach ($method->getParameters() as $param) {
                if (isset($this->actionParams[$param->name]) && strlen($this->actionParams[$param->name]))
                    $args[$param->name] = $this->actionParams[$param->name];
            }
